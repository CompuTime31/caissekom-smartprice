...
window.SmartPriceCloud={
  ...
  async getExistingMap(c=this.config()){const db=this.client(c),map=new Map();let from=0,size=1000;while(true){const{data,error}=await db.from('products').select('barcode,designation,price,active').range(from,from+size-1);if(error)throw new Error(this.explain(error,'Lecture du catalogue central impossible'));for(const x of data||[])map.set(String(x.barcode),x);if(!data||data.length<size)break;from+=size}return map},
  async syncCatalog(articles,{replaceMissing=false,onProgress=null,source='manual'}={},c=this.config()){
    const db=this.client(c),now=new Date().toISOString(),existing=await this.getExistingMap(c),rows=articles.map(a=>({barcode:String(a.code).trim(),designation:String(a.designation||'').trim(),price:Number(a.prix)||0,active:true,updated_at:a.updatedAt||now})).filter(x=>x.barcode&&x.designation);
    ...
    for(let i=0;i<rows.length;i+=500){const{error}=await db.from('products').upsert(rows.slice(i,i+500),{onConflict:'barcode',ignoreDuplicates:false});if(error)throw new Error(this.explain(error,`Envoi impossible (lot ${Math.floor(i/500)+1})`));onProgress?.(Math.min(85,Math.round(((i+500)/Math.max(rows.length,1))*85)))}
    let deactivated=0;if(replaceMissing){const incoming=new Set(rows.map(x=>x.barcode)),missing=[...existing.keys()].filter(x=>!incoming.has(x));deactivated=missing.length;for(let i=0;i<missing.length;i+=500){const{error}=await db.from('products').update({active:false,updated_at:now}).in('barcode',missing.slice(i,i+500));if(error)throw new Error(this.explain(error,'Désactivation des anciens articles impossible'))}}
    const summary={total:rows.length,added,updated,deactivated,mode:replaceMissing?'replace':'merge',source,created_at:now};
    const{error:logError}=await db.from('imports').insert({filename:source,total_rows:rows.length,added_rows:added,updated_rows:updated,error_rows:0,deactivated_rows:deactivated,mode:summary.mode,created_at:now});if(logError)console.warn('Journal import non enregistré',logError);
