...
const productFile=join(dataDir,'products.json');
const syncLogFile=join(dataDir,'sync-history.json');
const auditFile=join(dataDir,'audit-history.json');
const backupDir=join(dataDir,'backups');
mkdirSync(dataDir,{recursive:true});
mkdirSync(backupDir,{recursive:true});
const port=Number(process.env.SMARTPRICE_PORT||8080);
...
const json=(res,status,payload)=>{res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','Access-Control-Allow-Origin':'*'});res.end(JSON.stringify(payload));};
const readJson=(file,fallback)=>{try{return JSON.parse(readFileSync(file,'utf8'))}catch{return fallback}};
const writeJson=(file,value)=>writeFileSync(file,JSON.stringify(value,null,2));
const readProducts=()=>readJson(productFile,[]);
const readConfig=()=>readJson(configFile,{storeName:'Mon magasin',address:'',phone:'',wifiSsid:'',wifiSecurity:'WPA',mode:'Local',setupComplete:false,port});
...
setInterval(()=>{try{createAutomaticBackup('daily')}catch(e){console.error('Sauvegarde automatique impossible',e)}},24*60*60*1000);
...
const server=http.createServer(async(req,res)=>{
 ...
 if(url.pathname==='/api/products'&&req.method==='GET'){const all=readProducts();const q=String(url.searchParams.get('q')||'').toLowerCase();const category=String(url.searchParams.get('category')||'');const sort=String(url.searchParams.get('sort')||'designation');const page=Math.max(1,Number(url.searchParams.get('page')||1));const pageSize=Math.min(200,Math.max(1,Number(url.searchParams.get('pageSize')||50)));let products=all.filter(p=>(!q||String(p.designation).toLowerCase().includes(q)||String(p.barcode).includes(q))&&(!category||String(p.category||'Non classé')===category));products.sort((a,b)=>sort==='price'?Number(a.price)-Number(b.price):sort==='stock'?Number(a.stock)-Number(b.stock):sort==='barcode'?String(a.barcode).localeCompare(String(b.barcode)):String(a.designation).localeCompare(String(b.designation),'fr'));return json(res,200,{products:products.slice((page-1)*pageSize,page*pageSize),total:products.length,page,pageSize,categories:[...new Set(all.map(p=>p.category||'Non classé'))].sort()});}
 if(url.pathname==='/api/products'&&req.method==='POST'){try{const p=await body(req);if(!p.barcode||!p.designation)return json(res,422,{ok:false,error:'Code-barres et désignation obligatoires'});const products=readProducts();if(products.some(x=>x.barcode===String(p.barcode)))return json(res,409,{ok:false,error:'Code-barres déjà existant'});const item={id:Date.now(),barcode:String(p.barcode).trim(),designation:String(p.designation).trim(),price:Number(p.price)||0,stock:Number(p.stock)||0,category:String(p.category||'Non classé'),family:String(p.family||''),status:'Créé',updatedAt:new Date().toISOString()};products.push(item);writeJson(productFile,products);audit('PRODUCT_CREATED',{id:item.id,barcode:item.barcode});return json(res,201,{ok:true,product:item})}catch(e){return json(res,400,{ok:false,error:String(e)})}}
 const productMatch=url.pathname.match(/^\/api\/products\/(\d+)$/);
 if(productMatch&&req.method==='PUT'){try{const id=Number(productMatch[1]);const patch=await body(req);const products=readProducts();const index=products.findIndex(p=>Number(p.id)===id);if(index<0)return json(res,404,{ok:false,error:'Article introuvable'});const barcode=String(patch.barcode??products[index].barcode).trim();if(products.some((p,i)=>i!==index&&p.barcode===barcode))return json(res,409,{ok:false,error:'Code-barres déjà existant'});products[index]={...products[index],...patch,id,barcode,status:'Modifié',updatedAt:new Date().toISOString()};writeJson(productFile,products);audit('PRODUCT_UPDATED',{id,barcode});return json(res,200,{ok:true,product:products[index]})}catch(e){return json(res,400,{ok:false,error:String(e)})}}
 if(productMatch&&req.method==='DELETE'){const id=Number(productMatch[1]);const products=readProducts();const found=products.find(p=>Number(p.id)===id);if(!found)return json(res,404,{ok:false,error:'Article introuvable'});writeJson(productFile,products.filter(p=>Number(p.id)!==id));audit('PRODUCT_DELETED',{id,barcode:found.barcode});return json(res,200,{ok:true});}
 if(url.pathname==='/api/products/bulk'&&req.method==='POST'){try{const payload=await body(req);const ids=new Set((payload.ids||[]).map(Number));if(!ids.size)return json(res,422,{ok:false,error:'Aucun article sélectionné'});let products=readProducts();const before=products.length;if(payload.action==='delete')products=products.filter(p=>!ids.has(Number(p.id)));else if(payload.action==='category')products=products.map(p=>ids.has(Number(p.id))?{...p,category:String(payload.category||'Non classé'),status:'Modifié',updatedAt:new Date().toISOString()}:p);else return json(res,422,{ok:false,error:'Action groupée inconnue'});writeJson(productFile,products);audit('PRODUCT_BULK_ACTION',{action:payload.action,requested:ids.size,affected:before-products.length||ids.size});return json(res,200,{ok:true,total:products.length})}catch(e){return json(res,400,{ok:false,error:String(e)})}}
 if(url.pathname==='/api/public/lookup'&&req.method==='GET'){const barcode=String(url.searchParams.get('barcode')||'').trim();const item=readProducts().find(p=>String(p.barcode)===barcode);if(!item)return json(res,404,{ok:false,error:'Article introuvable'});return json(res,200,{ok:true,product:{designation:item.designation,barcode:item.barcode,price:item.price,status:item.status,category:item.category||null}});}
